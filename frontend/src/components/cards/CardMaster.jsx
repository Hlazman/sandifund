...
import { ImageBox, Field } from "./_CardParts";
...
export default function CardMaster({ master }) {
  ...
  const productsCount = useMemo(() => {
    ...
    return arr.filter(p => p?.state === "inStock" || p?.state === "booked").length;
  }, [master]);
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 flex flex-col">
      <div className="mb-3">
        {imgSrc ? (
          <img src={imgSrc} alt={master?.name || ""} className="w-full aspect-[4/3] object-cover rounded-xl border border-gray-200" />
        ) : (<ImageBox text="no photo" />)}
      </div>

      <div className="flex items-start justify-between gap-3">
        <h3 className="text-lg font-semibold leading-tight break-words">{master?.name}</h3>
        <span className="text-sm text-gray-600">
          {t("card.master.products", "products")}: <b>{productsCount}</b>
        </span>
      </div>

      {master?.description ? <div className="mt-2"><RichText value={master.description} /></div> : null}

      <div className="mt-2 grid gap-1.5">
        <Field label="Email" value={master?.email} href={master?.email ? `mailto:${master.email}` : undefined} />
        <Field label="WhatsApp" value={master?.whatsapp} href={master?.whatsapp ? `https://wa.me/${master.whatsapp}` : undefined} />
      </div>

      <div className="mt-3">
        <Link to={`/masters/${master.documentId}`} className="w-full inline-flex items-center justify-center px-3 py-2 rounded-xl border border-gray-200 bg-indigo-600 text-white hover:bg-indigo-700">
