...
//                     email: m.email,
//                     whatsapp: m.whatsapp,
//                     onPress: undefined, // уже на странице мастера, ссылка не нужна
//                   }}
//                 />
//               );
//             })}
...
export default function Master() {
  ...
  const m = data?.master;
  const photoUrl = m?.photo?.url
    ? m.photo.url.startsWith("http")
      ? m.photo.url
      : `${API_BASE}${m.photo.url}`
    : null;

  const products = Array.isArray(m?.products) ? m.products : [];
  const productsCount = products.length;
  const descriptionText = m ? slateToText(m.description) : "";
            ...
            {products.map((p) => {
              const imgUrl = p?.image?.url
                ? p.image.url.startsWith("http")
                  ? p.image.url
                  : `${API_BASE}${p.image.url}`
                : null;

              const masterImage = photoUrl;
              ...
              return (
                <CardProduct
                  ...
                  price={typeof p.price === "number" ? p.price : undefined}
                  donationPercent={
                    typeof p.donationPercent === "number"
                      ? p.donationPercent
                      : undefined
                  }
                  status={p.state}
                  master={{
                    ...
                    email: m.email,
                    whatsapp: m.whatsapp,
                    // onPress не нужен — уже на странице мастера
                  }}
                />
              );
            })}
